...
func TestPawnMoves(t *testing.T) {
	start := StartPosition()
	runCases(t, []genCase{
		{"pawn first move", start, "e2", "e3 e4"},
		{"pawn first move blocked on e4", place(map[string]Piece{"e2": wPawn, "e4": wKnight}), "e2", "e3"},
		{"pawn second move", place(map[string]Piece{"e3": wPawn}), "e3", "e4"},
		{"pawn capture moves", place(map[string]Piece{"a2": wPawn, "b3": bPawn}), "a2", "a3 a4 b3"},
		{"black pawn first move", start, "e7", "e5 e6"},
		{"pawn first move blocked on e5", place(map[string]Piece{"e7": bPawn, "e5": bPawn}), "e7", "e6"},
		{"cannot capture own piece", place(map[string]Piece{"e4": wPawn, "d5": wPawn, "f5": bPawn}), "e4", "e5 f5"},
		{"stranded on last rank", place(map[string]Piece{"e8": wPawn}), "e8", ""},
	})
}
